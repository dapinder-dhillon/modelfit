...
from .providers import EFFORT_BUDGETS, MODELS, _pass_probability, cost_usd


def projected_cell(text: str, model: str, effort: str, quadrant: str) -> tuple[float, float]:
    ...
    prob = _pass_probability(quadrant, MODELS[model], effort)
    return prob, cost_usd(model, in_tokens, out_tokens)


def chart(est: Estimate, path: str) -> bool:
    ...
    for model in MODELS:
        ...
        for effort in efforts:
            prob, cost = projected_cell(est.text, model, effort, est.quadrant)
            xs.append(cost * 100)
            ys.append(prob * 100)
        ax.plot(xs, ys, marker="o", lw=1.2, ms=5, alpha=0.85, label=model.replace("claude-", ""))
    ...
    start_prob, start_cost = projected_cell(
        est.text, est.start_model, est.start_effort, est.quadrant
    )
    ax.scatter(
        [start_cost * 100],
        [start_prob * 100],
        ...
        linewidths=1.8,
        zorder=4,
        label=f"advised start: {est.start_model.replace('claude-', '')} @ {est.start_effort}",
    )
